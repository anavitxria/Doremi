...
function autenticarProfessor(req, res) {
    ...
    if (email == undefined) {
        ...
        usuarioModel.autenticarProfessor(email, senha)
            .then(
                function (resultadoAutenticar) {
                    console.log(`\nResultados encontrados: ${resultadoAutenticar.length}`);
                    console.log(`Resultados: ${JSON.stringify(resultadoAutenticar)}`); // transforma JSON em String
...
function obterDadosAluno(req, res) {
    var idProfessor = req.params.idProfessor;

    if (!idProfessor) {
        return res.status(400).send("Parâmetro idProfessor não foi fornecido!");
    }

    usuarioModel.obterDadosAluno(idProfessor).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
